...
export function FoodBar({ currentFood, icon }: FoodBarProps) {
  // We only display a maximum number of NEXT_PUBLIC_FOOD_CAP for food items
  const foodNumber =
    currentFood >= Number(NEXT_PUBLIC_FOOD_CAP)
      ? Number(NEXT_PUBLIC_FOOD_CAP)
      : currentFood;
  const fullIcons = Math.floor(foodNumber / 2);
  const emptyIcons = Math.floor(
    (Number(NEXT_PUBLIC_FOOD_CAP) - foodNumber) / 2,
  );
